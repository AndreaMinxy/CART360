...
//this line of code sets up the pin number 
int LED1 = 4;
int LED2 = 3;
int LED3 = 2;
int LED4 = 1;
...
//this block of code is setting up the letters and the index (order of tbe light) of the LEDs that we can use 
int a[] = {1, 6, 26, 6, 1};
int b[] = {31, 21, 21, 10, 0};
...
//this sets up the punctuation symbols
int eos[] = {0, 1, 0, 0, 0}; 
int excl[] = {0, 29, 0, 0, 0};
int ques[] = {8, 19, 20, 8, 0};
int equal[] = {10, 10, 10, 10, 10 }; //this is the new symbol added

void displayLine(int line) //saving the int value and holding it as myLine
{ //Displays the sequence of which LED WILL BE ON AND off
  int myline;
  myline = line;
...
void displayChar(char c)
{
  ...
  if (c == '='){for (int i = 0; i <5; i++){displayLine(equal[i]);delay(delayTime);}displayLine(0);}
  delay(charBreak);
}

void displayString(char* s)
{
  for (int i = 0; i<=strlen(s); i++) //i starts at the value of zero, when i is less than or equal to strlen(s), increase zero by 1
  {
    displayChar(s[i]);  //display the value of i
  }
}

void loop()
{   
  displayString("andrea=andrea "); //the exposure will display the word yay
}
